...
function draw() {
    ...
    text("Checking " + totalPerms + " different combinations using brute force lexigraphic ordering.\n" + s + "% complete.\n" + Math.floor(permCount / (millis() / 1000)) + " routes per second.", 11, currBestY1 - (textAscent() * 2) - 1);

    //create a new array of cities in the order for this permutation
...
function bestSolGraph() {
    ...
    bestSols.sols.forEach(function(elt, index) {
        ...
        let cy = map(currDist, 0, maxDist, solGraphY2 - textAscent(), solGraphY1);
        if (currDist === shortestDist) {
            fill(199, 0, 199, 199);
            textSize(14);
            noStroke();
        ...
        stroke(sClr);
        strokeWeight(sWgt);
        line(cx, solGraphY2 - textAscent(), cx, cy);
    ...
    for (let i = 0; i < 100; i += 1) {
        ...
        let scaleY = solGraphY2 - textAscent();
        stroke(199, 199);
        strokeWeight(1);
        point(scaleX, scaleY);
    }
